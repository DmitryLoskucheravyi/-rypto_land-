...
const MENU = ['Basic', 'Intermediate', 'Advanced'];

// Step 1 — the cursor travels to "join", clicks, the menu drops in and each
...
export const SelectDemo = () => {
  ...
  const show = (delay: number) =>
    play
      ? { opacity: 1, y: 0, transition: { duration: DURATION.standard, ease: EASE, delay } }
      : {};
...
export const PriceDemo = ({ title }: { title?: string }) => {
  const { ref, play, reduce } = useStepPlay();
  // No specific amount here: pricing lives in Telegram, the demo shows the flow.
  const line = 'price: listed in Telegram';
      ...
      <Frame label="telegram">
        <div className="flex flex-col gap-3 font-mono text-xs">
          <motion.div
            ...
            animate={
              play
                ? { opacity: 1, y: 0, transition: { duration: DURATION.standard, ease: EASE } }
                : {}
            }
            className="text-ink-muted"
          >
            <span className="text-accent">&gt;</span> course: {title ?? 'basic'}
          </motion.div>
          ...
          <motion.div
            initial={{ opacity: 0 }}
            animate={
              done ? { opacity: 1, transition: { duration: DURATION.standard, ease: EASE } } : {}
            }
...
const FILES = ['module 01', 'module 02', 'module 03'];
        ...
        <div className="flex items-center gap-5">
          ...
          >
            ...
            <motion.path
              ...
              animate={
                play ? { rotate: reduce ? -28 : [0, -4, -28], y: reduce ? -3 : [0, 1, -3] } : {}
              }
