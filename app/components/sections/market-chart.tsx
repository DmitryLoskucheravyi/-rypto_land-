...
const GEOMETRY: Polyline[] = LINES.map((line) => {
  const width = VIEWPORT + PAN * line.speed;
  const pts = randomWalk({ ...line, width, height: HEIGHT });
  return buildPolyline(pts, width);
});

// Atmospheric only: a random walk started from a fixed seed value, never a real
// quote. Labelled "demo" on screen and generated client-side after mount, so
...
const Ticker = () => {
  ...
  return (
    <div className="mt-8 shrink-0 md:mt-0 md:text-right" aria-hidden="true">
      ...
      <div
        className={`font-mono text-2xl tabular-nums transition-colors duration-standard ease-premium sm:text-3xl md:text-5xl ${
          quote.up ? 'text-accent' : 'text-bear'
        }`}
      >
        {formatQuote(quote.value)}
      </div>
    </div>
  );
};
...
export const MarketChart = () => {
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      const draw = (p: number) => {
        ...
        const lead = LINES[0];
        const tipX = xAtLengthFraction(GEOMETRY[0], p * lead.speed) * scale - PAN * lead.speed * p;
  ...
  return (
    <section
      ...
      className="scroll-mt-24 overflow-hidden py-20 md:py-32"
    >
      ...
      <div ref={plotRef} className="relative mt-12 h-[180px] sm:h-[220px] md:mt-14 md:h-[300px]">
