...
// The two distances live in CSS (--stack-top / --stack-peek, globals.css) so a
// phone can park the cards tighter without duplicating the layout here.
const cardTop = (i: number) => `calc(var(--stack-top) + ${i} * var(--stack-peek))`;
...
export const HowItWorks = () => {
  ...
  // The demo never names a price: pricing lives in Telegram, and the card
  // shows the flow rather than a specific amount.
  const entry = courses.find((c) => c.tier === 'basic') ?? courses[0];
  const demos = [
    <SelectDemo key="select" />,
    <PriceDemo key="price" title={entry?.title} />,
  ...
  return (
    <section
      ...
      className="relative scroll-mt-24 pt-20 md:pt-32"
    >
      <div className="mx-auto max-w-container px-5 sm:px-6 md:px-8">
        <motion.h2
          ...
          variants={fadeUp}
          className="text-[1.6rem] font-semibold leading-tight sm:text-2xl md:text-[2.25rem] md:leading-[1.1]"
        >
      ...
      <div ref={stackRef} className="relative mt-14">
        ...
        {steps.map((step, i) => {
          ...
          return (
            <div
              ...
              style={{ top: cardTop(i), zIndex: i + 1 }}
            >
              <div className="mx-auto max-w-container px-5 sm:px-6 md:px-8 lg:pl-20">
                ...
                <div className="rounded-2xl bg-bg/90 shadow-[0_24px_70px_-28px_hsl(var(--bg))]">
                  <div
                    className="glass-panel relative flex min-h-[42vh] flex-col overflow-hidden rounded-2xl px-6 py-8 sm:px-8 sm:py-9 md:min-h-[44vh] md:px-12 md:py-11"
                    style={{
                      background:
                  ...
                  >
                    ...
                    <span
                      aria-hidden="true"
                      className="step-watermark pointer-events-none absolute -top-3 left-3 select-none font-display text-[5.5rem] font-extrabold leading-none tabular-nums sm:-top-4 sm:left-4 sm:text-[7rem] md:-top-6 md:left-8 md:text-[11rem]"
                    >
                      {pad(i + 1)}
                    </span>
                    ...
                    <div className="relative grid flex-1 items-center gap-8 md:grid-cols-2 md:gap-12">
                      <div>
                        <div className="flex items-center gap-3 sm:gap-4">
                          ...
                          <h3 className="text-lg font-medium sm:text-xl md:text-2xl">
                            {step.title}
                          </h3>
                        </div>
                        <p className="mt-5 text-[0.95rem] leading-relaxed text-ink-muted sm:mt-6 sm:text-base md:text-lg">
                          {step.body}
                        </p>
                      </div>
        ...
        <div className="h-[45vh] md:h-[70vh]" />
      </div>
    </section>
  );
};
